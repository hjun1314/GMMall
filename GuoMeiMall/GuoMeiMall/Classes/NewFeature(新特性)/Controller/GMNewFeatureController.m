...
@interface GMNewFeatureController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@end
static NSString *const GMNewFeatureCellID = @"DCNewFeatureCell";
@implementation GMNewFeatureController
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[GMNewFeatureCell class] forCellWithReuseIdentifier:GMNewFeatureCellID];
    }
...
- (UIPageControl *)pageControl
{
    if (!_pageControl && _imageArray.count != 0) {
        ...
        _pageControl.frame = CGRectMake(0, SCREEN_HEIGHT * 0.95, SCREEN_WIDTH, 35);
...
{
    GMNewFeatureCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:GMNewFeatureCellID forIndexPath:indexPath];
    
    cell.nfImageView.image = (iPhoneXScreen) ? [UIImage imageNamed:[NSString stringWithFormat:@"%@_x",_imageArray[indexPath.row]]]  : [UIImage imageNamed:_imageArray[indexPath.row]];
    ...
    cell.hideButtonClickBlock = ^{
        ...
        [weakself restoreRootViewController:[[GMTabController alloc] init]];
    };
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (scrollView.contentOffset.x >  (_imageArray.count - 1) * SCREEN_WIDTH) {
        [self restoreRootViewController:[[GMTabController alloc] init]];
    }
}
